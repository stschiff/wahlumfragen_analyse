function zweitstimmen_chart(svg) {
  ...
  var x = d3.time.scale().range([0, width]).domain([parseDate("27.09.2009"), parseDate("30.09.2013")]),
      ...
      y = d3.scale.linear().range([height, 0]).domain([0, 70]);
  ...
  var show = {"CDU":true, "SPD":true, "GRUENE":true, "FDP":true, "LINKE":true, "PIRATEN":false, "AFD":false};
  var election = {"CDU":41.5, "SPD":25.7, "LINKE":8.6, "GRUENE":8.4, "FDP":4.8, "AFD":4.7, "PIRATEN":2.2};
  var electionDate = "22.9.2013";
  ...
  d3.tsv("data/projection.txt", function(error, data) {
  
    partyData = partyColors.domain().map(function(name) {
      return {
        name: name,
        values: data.filter(function(d) {return dateFilter(name, parseDate(d.Date));}).map(function(d) {
          return {
  ...
  function updateHover() {
    ...
    hoverTextGroup.selectAll(".hoverTextPartyRange")
      .text(function(d, j) {
        ...
        return dateFilter(d.name, x0) ? d.name + ": " + Math.round(d.values[i].lower*10)/10 + "-" + Math.round(d.values[i].higher*10)/10 : "";
      });
  }
  ...
  function drawAxes() {
    ...
    focus.append("g")
      ...
      .text("90% Wahrscheinlichkeit Zweitstimmenergebnis (%)");
